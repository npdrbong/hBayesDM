#' @templateVar MODEL_FUNCTION omsfree_par4
...
#' @templateVar MODEL_NAME one more step version. Re-parameterized version of BART model with 4 parameters
...
#' @templateVar PARAMETERS \code{phi} (prior belief of balloon not bursting), \code{eta} (updating rate), \code{gam} (risk-taking parameter), \code{tau} (inverse temperature), \code{epsi} (free parameter for 'one more step')
