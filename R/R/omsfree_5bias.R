#' @templateVar MODEL_FUNCTION omsfree_5bias
#' @templateVar CONTRIBUTOR 
#' @templateVar TASK_NAME Balloon Analogue Risk Task, one more step version, and with free parameter
...
#' @templateVar PARAMETERS \code{phi} (prior belief of balloon not bursting), \code{tau} (inverse temperature), \code{gam} (risk-taking parameter), \code{alpha} (Learning Bias), \code{beta} (Perceptual Bias), \code{epsi} (free parameter for 'one more step')
