...
from hbayesdm.preprocess_funcs import omsfree_preprocess_func

__all__ = ['omsfree_5bias']


class Omsfree5Bias(TaskModel):
    def __init__(self, **kwargs):
        super().__init__(
            ...
            parameters_desc=OrderedDict([
                ...
                ('alpha', 'Learning Bias'),
                ('beta', 'Perceptual Bias'),
                ('epsi', 'free parameter for 'one more step''),
            ]),
    ...
    _preprocess_func = omsfree_preprocess_func
...
def omsfree_5bias(
    ...
    """Balloon Analogue Risk Task, one more step version, and with free parameter - one more step version. Bayesian updating of risk rewritten with 'Bias' term, with 'gam'

    Hierarchical Bayesian Modeling of the Balloon Analogue Risk Task, one more step version, and with free parameter 
    using one more step version. Bayesian updating of risk rewritten with 'Bias' term, with 'gam' [Kim2020]_ with the following parameters:
    "phi" (prior belief of balloon not bursting), "tau" (inverse temperature), "gam" (risk-taking parameter), "alpha" (Learning Bias), "beta" (Perceptual Bias), "epsi" (free parameter for 'one more step').

    

    
    .. [Kim2020] Kim, M., Kim, S., Lee, K. U., & Jeong, B. (2020). Pessimistically biased perception in panic disorder during risk learning. Depression and anxiety, 37(7), 609-619.
    ...
    **tab-delimited** text file, whose rows represent trial-by-trial observations
    ...
    For the Balloon Analogue Risk Task, one more step version, and with free parameter, there should be 4 columns of data
    with the labels "subjID", "pumps", "explosion", "oms". It is not necessary for the columns to be
    ...
    data
        ...
        Data columns should be labeled as: "subjID", "pumps", "explosion", "oms".
    niter
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('omsfree_5bias').
    ...
    .. code:: python
        ...
        output = omsfree_5bias(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
