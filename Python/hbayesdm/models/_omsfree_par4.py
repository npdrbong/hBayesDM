...
__all__ = ['omsfree_par4']


class OmsfreePar4(TaskModel):
...
def omsfree_par4(
    ...
    """Balloon Analogue Risk Task, one more step version - one more step version. Re-parameterized version of BART model with 4 parameters

    Hierarchical Bayesian Modeling of the Balloon Analogue Risk Task, one more step version 
    using one more step version. Re-parameterized version of BART model with 4 parameters [van_Ravenzwaaij2011]_ with the following parameters:
    "phi" (prior belief of balloon not bursting), "eta" (updating rate), "gam" (risk-taking parameter), "tau" (inverse temperature), "epsi" (free parameter for 'one more step').
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('omsfree_par4').
    ...
    .. code:: python
        ...
        output = omsfree_par4(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
