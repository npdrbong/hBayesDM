...
from hbayesdm.preprocess_funcs import oms_preprocess_func

__all__ = ['oms_5bias']


class Oms5Bias(TaskModel):
    ...
    _preprocess_func = oms_preprocess_func
...
def oms_5bias(
    ...
    """Balloon Analogue Risk Task, one more step version - one more step version. Bayesian updating of risk rewritten with 'Bias' term, with 'gam'

    Hierarchical Bayesian Modeling of the Balloon Analogue Risk Task, one more step version 
    ...
    "phi" (prior belief of balloon not bursting), "tau" (inverse temperature), "gam" (risk-taking parameter), "alpha" (Learning Bias), "beta" (Perceptual Bias).
    ...
    **tab-delimited** text file, whose rows represent trial-by-trial observations
    ...
    For the Balloon Analogue Risk Task, one more step version, there should be 4 columns of data
    ...
    **additional_args
    ...
    model_data
        ...
        - ``model``: String value that is the name of the model ('oms_5bias').
    ...
    .. code:: python
        ...
        output = oms_5bias(data='example', niter=2000, nwarmup=1000, nchain=4, ncore=4)
