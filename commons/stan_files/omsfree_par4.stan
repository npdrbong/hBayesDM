...
transformed parameters {
  ...
  epsi = Phi_approx(mu_pr[5] + sigma[5] * epsi_pr);
}
...
model {
  ...
  for (j in 1:N) {
    ...
    for (k in 1:Tsubj[j]) {
      ...
      // if explosion == 0 -> One more step!
      if (explosion[j, k] == 1) {
        n_succ += pumps[j, k] - explosion[j, k];
        n_pump += pumps[j, k];
      } else {
        n_succ += pumps[j, k] + epsi[j] * (1 - oms[j, k]);
        n_pump += pumps[j, k] + epsi[j];
      }
    }
  }
}
...
generated quantities {
  ...
  real<lower=0, upper=1> mu_epsi = Phi_approx(mu_pr[5]);
