...
transformed parameters {
  ...
  gam = exp(mu_pr[5] + sigma[5] * gam_pr);
  epsi = Phi_approx(mu_pr[6] + sigma[6] * epsi_pr);
}
...
model {
  ...
  for (j in 1:N) {
    real p_risk[T +1];
    real cum_pump = 0; // Number of total pumps, but in 'omsfree' model, we asssumed that agent regard additional pump as real number.
    p_risk[1] = phi[j];
    for (k in 1:Tsubj[j]) {
      real Nsucc;
      real weight;
      real evidence;
      real omega = - gam[j] / log( p_risk[k] ); // Optimal Number of Pumps 
      ...
      if (explosion[j, k]==1) {
        ...
        Nsucc = pumps[j,k] + epsi[j] * (1 - oms[j,k]); // pumps += 1 and reflect OMS
        weight = (pumps[j,k] + epsi[j]) * inv(pumps[j,k] + epsi[j] + cum_pump);
        evidence = Nsucc * inv(pumps[j,k] + epsi[j]);  
        // update p_risk -- risk learning
        if (pumps[j,k]>0){
          p_risk[k+1] = (evidence*beta[j])*(weight*alpha[j]) + (1-weight*alpha[j])*p_risk[k] ;      
        } else{
...
generated quantities {
  ...
  real<lower=0> mu_gam = exp(mu_pr[5]);
  real<lower=0, upper=1> mu_epsi = Phi_approx(mu_pr[6]);
  ...
  for (j in 1:N)
    ...
    for (j in 1:N) {
      real cum_pump = 0; // Number of total pumps
      real p_risk[T+1];
      log_lik[j] = 0;
      p_risk[1] = phi[j];
      for (k in 1:T) {
